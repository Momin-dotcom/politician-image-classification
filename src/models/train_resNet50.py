"""
train_resnet50.py
ResNet-50 transfer learning for Pakistani politician classification.

Phases:
  Phase 1 — Freeze all pretrained layers, train only the new head.
  Phase 2 — Unfreeze everything, fine-tune with a lower learning rate.

Usage (local, after downloading results from Colab):
    python src/models/train_resnet50.py
"""
...
def load_datasets(dataset_path, train_tf, val_tf, batch_size):
    """Load train/val/test ImageFolder datasets and return DataLoaders."""
...
def build_resnet50(num_classes, freeze_layers=True):
    """
    Build ResNet-50 with ImageNet pretrained weights.
    Replaces the final FC layer with a custom head for num_classes.
...
def train_model(model, model_name, train_loader, val_loader, config, device):
    """
    Two-phase training:
      Phase 1: Train frozen backbone with new head only.
      Phase 2: Unfreeze all layers and fine-tune with lower LR.

    Returns: model, history dict, best_val_acc, best_model_path
    """
...
if __name__ == '__main__':
    ...
    model = build_resnet50(cfg['num_classes'], freeze_layers=True)
    train_model(model, 'ResNet50', train_loader, val_loader, cfg, device)